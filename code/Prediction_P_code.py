#Code written by: Shuya Guo, Kexin Guan, Yizhen Situ

##General overview
##Pred_P is an interactive desktop application for predicting permeability coefficient of gas molecules in MOF or other porous crystal materials. The core of its computation is our trained XGBoost model. The following is the source code for the interface design.
...
model = joblib.load("model/xgboost.pt") ## The detailed code for model training is in “XGBoost.py”

## The layout design of the main interface
root= tk.Tk()
root.title("Predict permeability of material on XGBoost")
...
label_B = tk.Label(root,font=('microsoft yahei',9),text='Author: Shuya Guo, Kexin Guan, Yizhen Situ, Zhiwei Qiao, Guangzhou University')
...
global tk_image 
photo1 = Image.open("Img/full_name.tif")
w, h = photo1.size       
...
global tk_image 
photo2 = Image.open("Img/sample_file.tif")  
w, h = photo2.size     
...
label_Z = tk.Label(root,font=('microsoft yahei',13),text='Predict permeability of material')
...
## Main interface for input of nine descriptor values (a single molecule permeability)
def values():       
...
## XGB Algorithm (The predictions of a single molecule permeability)   
    lgP = model.predict([[New_HVF, New_PLD, New_LCD, New_Density, New_VSA, New_Dia,
    ...
    P3= P2.split('E')  
    if (P3[1])[0] == "-":
        P3= P3[0]+" x 10^"+ P3[1].lstrip('0')
    else:
        P3=P3[0]+" x 10^"+ (P3[1])[1:].lstrip('0')    
    
    ## label of the predicted result
    Prediction_result  = (P3)   
...
button1 = tk.Button (root,font=('microsoft yahei',10), text='Predicted P',command=values) 
canvas1.create_window(130, 380, window=button1)

## Batch prediction of material permeability
label_Z1 = tk.Label(root,font=('microsoft yahei',12),text='Batch prediction of material permeability')
...
## Output XGB model prediction results
def print_file():
